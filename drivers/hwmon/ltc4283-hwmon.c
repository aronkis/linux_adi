				 ...
				 u32 channel, long *val)
{
	...
	case hwmon_in_input:
		scoped_guard(mutex, &st->lock) {
			...
			return ltc4283_hwmon_read_in_vals(st, attr, channel,
							  val);
		}
	case hwmon_in_highest:
	case hwmon_in_lowest:
...
static int ltc4283_hwmon_reset_in_hist(struct ltc4283_hwmon *st, u32 channel)
{
	...
	/* Then, let's also clear possible fet_bad faults */
...
static int ltc4283_hwmon_write_curr_history(struct ltc4283_hwmon *st)
{
	...
	return regmap_clear_bits(st->map, LTC4283_FAULT_LOG,
				 LTC4283_OC_FAULT_MASK | LTC4283_FET_SHORT_FAULT_MASK);
}
					   ...
					   u32 attr, int channel)
{
	...
	if (channel >= LTC4283_HWMON_ADIO_1 && channel <= LTC4283_HWMON_ADIO_4) {
		...
		channel += LTC4283_ADIOX_START_NR - LTC4283_HWMON_ADIO_1;
...
static const struct hwmon_channel_info * const ltc4283_hwmon_info[] = {
	...
	HWMON_CHANNEL_INFO(curr,
			   ...
			   HWMON_C_FAULT | HWMON_C_RESET_HISTORY |
			   HWMON_C_LABEL),
...
DEFINE_DEBUGFS_ATTRIBUTE(ltc4283_fet_short_crit_fault_log,
...
static int ltc4283_hwmon_set_fault_log(void *arg, u64 val)
{
	struct ltc4283_hwmon *st = arg;

	return regmap_write(st->map, LTC4283_FAULT_LOG, val);
}

static int ltc4283_hwmon_show_fault2_log(void *arg, u64 *val)
{
	struct ltc4283_hwmon *st = arg;
	u32 __val;
	int ret;

	ret = regmap_read(st->map, LTC4283_FAULT_LOG, &__val);
	if (ret)
		return ret;
...
DEFINE_DEBUGFS_ATTRIBUTE(ltc4283_fault_log,
			 ltc4283_hwmon_show_fault2_log, ltc4283_hwmon_set_fault_log, "%llx\n");
				 ...
				 const struct device *hwmon)
{
	...
	debugfs_create_file_unsafe("curr1_fet_short_fault_log", 0400, dentry, st,
				   &ltc4283_fet_short_crit_fault_log);

	debugfs_create_file_unsafe("fault_log", 0600, dentry, st,
				   &ltc4283_fault_log);

	/* Missing EXTERNAL faults and clarify VOUT/PGI */
}
